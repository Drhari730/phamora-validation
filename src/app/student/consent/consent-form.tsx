...
const infoPoints = [
  ...
  },
  {
    ...
    text: "You are identified only by an anonymous participant code in all survey data. Your email is used only to send you a welcome message and study updates.",
  },
  {
    icon: LogOut,
...
export function ConsentForm({ ethicsRef }: { ethicsRef: string | null }) {
  ...
  function handleAgree() {
    ...
    startTransition(async () => {
      const result = await submitConsent({ informationRead: read, voluntaryAgree: agree, email });
      if (result.ok) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-2xl px-6 py-10">
        ...
        <div className="mb-6 rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-8">
          ...
          <div className="rounded-2xl border border-border bg-secondary p-4 text-xs text-secondary-foreground">
            {ethicsRef ? (
              <>Ethics approval has been obtained from the Institutional Ethics Committee (reference: <span className="font-mono">{ethicsRef}</span>).</>
            ) : (
        ...
        <div className="mb-6 rounded-3xl border border-border bg-card p-6 shadow-sm">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Used only to send you a welcome message now and, if needed, study
            reminders — never shown alongside your survey responses.
          </p>
        </div>
        ...
        <Button
          ...
          size="lg"
          disabled={!read || !agree || !emailValid || pending}
          onClick={handleAgree}
        >
